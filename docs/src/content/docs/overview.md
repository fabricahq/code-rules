...
The offline builder validates rule inputs and generates their effective output. Generated-file consistency checks remain planned. The proposed `code-rules check` command checks that generated rules match their inputs; it does not check whether application code follows those rules.
