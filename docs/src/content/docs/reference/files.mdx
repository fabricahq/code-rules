...
Each entry contains a group name, **Description**, **When to read this group**, and an explicit **Open group** link. If no groups are selected, the root index says so explicitly.
When local metadata exists for a group, it supplies the complete discovery guidance. Otherwise, contributing library cues remain separate and labeled by source.
One instruction above the entries directs the agent to open relevant group indexes and read applicable rules in full.
Inspect the adjacent `provenance.json` for source versions and rule origins.
...
A small group includes every active rule's complete resolved definition inline.
...
Full-rule group pages have an 8 KiB inline limit and must also fit the 750-line page limit. These are rendering defaults, not CLI flags. The internal renderer accepts an explicit inline limit; zero forces summaries.
