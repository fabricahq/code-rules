...
The `code-rules sync` command updates your project's imported library files and regenerates the guidance your agents read. Run it after adding a library, changing its revision or selected groups, or to fetch updates allowed by your configured version range.

This page explains how to run sync, what it changes, and what to do when files are missing, outdated, or left by an interrupted update. It also explains when `code-rules build` or `code-rules check` is enough. For how Code Rules selects and combines library rules, see [How imports work](/reference/imports/).

## Choose the right command

- **`code-rules sync`**: Fetch selected library revisions or restore imported files. Sync validates the library files and regenerates agent guidance.
- **`code-rules build`**: Apply local rule changes or exceptions using the library files you already have. Build regenerates guidance without contacting a repository.
- **`code-rules check`**: Find out whether generated guidance and the project README are up to date. Check validates stored inputs and output, reports problems, and leaves files unchanged.
...
By default, the commands read `.code-rules/config.json` relative to your working directory. Use `--config` to select a different configuration file:

```sh
code-rules sync --config path/to/code-rules/config.json
```

The configuration file and its containing directory must already exist. Code Rules keeps `local/`, `vendor/`, and `generated/` beside that file. The paths below are relative to this directory.
...
After a successful update, sync and build report sorted lists of added, changed, and removed paths. In JSON, these lists use the keys `added`, `changed`, and `removed`.
...
Use the problem reported by `code-rules check` to choose a repair:

| Problem | What to do |
| --- | --- |
| Generated guidance is missing or outdated, but imported files are valid. | Run `code-rules build`. |
| Imported files are missing, modified, or no longer match your configured sources or groups. | Run `code-rules sync`. Preserve any edits you intended to keep as local rules first. |
| The managed project README is missing or outdated. | Run `code-rules init` to refresh it. |
| Configuration or rule metadata is invalid. | Correct the reported input, then retry the appropriate command. |

Run `code-rules check` again after repairing the problem.
