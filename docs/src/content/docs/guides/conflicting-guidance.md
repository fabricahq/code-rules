...
The prompt identifies the generated index, every effective group file, and the snapshot's source refs and resolved commits.
...
Read code-rules/generated/RULES.md and every group file it lists.
...
Keep edits in source files and configuration, then regenerate the aggregates.
...
Inspect the regenerated group files, then generate a fresh conflict-review prompt for that snapshot.
