...
- Require nonblank `name` and `description` strings, preserving their exact text.
- Require a `whenToRead` array of distinct, nonblank strings. Preserve order and surrounding whitespace; an empty array is valid.
...
The page opens with group metadata selected. Try the valid document, empty guidance, preserved order/whitespace, missing name, null list, duplicate guidance, blank-before-duplicate, license declaration, unknown field, and malformed JSON presets.
...
The shared suite contains 51 metadata cases plus 87 identity cases. Metadata expectations match TypeScript exactly; this slice introduces no approved diagnostic differences.
...
- The 138 shared cases passed with the existing 60 approved identity diagnostic differences; there are no newly approved differences.
