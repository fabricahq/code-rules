...
- Apply stages all output, checks caller inputs and target digests, then writes a
  version-1 journal before any live replacement. Failed operations restore prior
  trees when doing so cannot destroy later edits.
- Recovery validates every restoration first. Changed current output or backups
...
- This does not promise simultaneous visibility across two renames or durability
  ...
  while a writer is active is unsupported. Root handles confine filesystem access.

The TypeScript version-1 journal layout and tree digest are retained, including
...
labelled filesystem evidence captured after the failed call.
...
Synthetic interruption tests cover post-interruption edits and committed cleanup.
