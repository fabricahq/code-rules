...
5. **Library authoring, implemented in the next PR:** provide `library init`, `library add group`, `library add rule`, and `library check`, reusing authoring helpers. Add library-wide manifest, license, and notice setup. Validate an authored library by importing it into a consuming project.
