...
4. **Project setup, implemented in the PR stacked on #6:** provide `init`, `local add group`, `local add rule`, and `add source`. Scaffold `.code-rules/local/README.md`, link canonical authoring guidance, preserve existing files, and offer missing-group creation interactively. Noninteractive commands take explicit inputs. Share authoring helpers and templates. Validate the empty-repo to local-rules to imported-library journey.
5. **Library authoring, PR #8:** provide `library init`, `library add group`, `library add rule`, and `library check`, reusing authoring helpers. Add library-wide manifest, license, and notice setup. Validate an authored library by importing it into a consuming project.
...
Release packaging follows PR #8 with Node.js distribution, packed-executable checks, a versioned publishing workflow, and installation docs. Account setup, tool licensing, and the real-project pilot are explicit publication gates; see [Releasing](releasing.md).
