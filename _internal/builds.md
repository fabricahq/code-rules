...
- `render.ts` builds the root index, adaptive group pages, individual resolved definitions, library READMEs, and provenance without mutating resolved groups.
...
The builder returns paths relative to `generated/`. Group pages live under `groups/<group-id>.md`, while individual resolved definitions live under `rules/`.
...
The snapshot envelope contains `repository`, either `ref` or `version`, `resolvedCommit`, `groups`, and `files`. Version selections also require `resolvedTag` and `resolvedVersion`.
...
File-handling helpers are responsible for mapping verified `_source.json` records into that envelope.
