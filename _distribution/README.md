...
The public [Fabrica tap](https://github.com/fabricahq/homebrew-tap) is active. The release integration is [merged](https://github.com/fabricahq/code-rules/pull/47), and hourly polling has been removed. A [manual dispatch after the switch](https://github.com/fabricahq/homebrew-tap/actions/runs/35426025303) succeeded with no formula to publish because Code Rules has no stable release yet. The first stable release will be picked up automatically. The hosted installer endpoint and end-to-end installation still need validation.
...
4. **Fabrica Homebrew Releaser** is configured using [the app setup below](#release-trigger-app). After publication, the release workflow dispatches **Update Code Rules** directly. To retry a failed dispatch or update, run that tap workflow manually. Releases remain independent of tap update failures.
...
Validate app authentication by dispatching the live tap workflow and checking that its actor is the app. This can run before the first stable release; the updater will succeed without creating a formula. The live tap matches the bootstrap workflow and has no schedule. Keep manual dispatch for recovery.
