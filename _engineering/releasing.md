...
- Changes to the managed project README format are breaking changes, including the `CODE_RULES.md` variant used with custom configuration locations. Before `1.0.0`, release them in a new minor version; from `1.0.0`, use a new major version. Describe the format change and automatic guide refresh in the release notes. Build and sync replace an older, unedited guide with the template bundled in the selected CLI version; they refuse to overwrite manual edits.
