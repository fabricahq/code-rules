/** @fileoverview Provides shared input guards, path checks, and group metadata parsing for library and project formats. */
...
type FileContents = Readonly<Record<string, string>>;

/** A format error with a source or field location, independent of the calling subsystem. */
...
/** Throw an invalid-input ValidationError with the location prefixed to the supplied message. */
export function invalid(location: string, message: string): never {
  throw new ValidationError(`${location}: ${message}`);
}

/** Copy own enumerable properties from a non-null, non-array object, or throw an invalid-input ValidationError. */
...
/** Return non-blank text unchanged, including surrounding whitespace; throw an invalid-input ValidationError otherwise. */
...
/** Return unique, non-blank strings in input order; throw an invalid-input ValidationError for invalid or duplicate entries. */
...
/** Return a contained relative path unchanged; reject absolute paths, empty/dot segments, backslashes, and control characters with ValidationError. */
...
/** Return the group ID from a contained Markdown rule path, or throw an invalid-input ValidationError. */
...
/** Parse JSON as unknown, wrapping syntax failures in an invalid-input ValidationError at the supplied location. */
...
/** Return a path-sorted map of text files; reject invalid paths or non-string contents with ValidationError. */
...
/** Return the file text, including empty text; throw an invalid-input ValidationError when the path is absent. */
...
/** Parse group selection guidance, preserving when-to-read order; throw ValidationError for invalid JSON or required fields. */
